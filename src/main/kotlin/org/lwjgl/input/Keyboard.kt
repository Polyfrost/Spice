...
import wtf.zani.spice.input.getKeyboard
import wtf.zani.spice.input.keyboardHandler

@Suppress("unused")
object Keyboard {
    @JvmStatic
    fun isCreated(): Boolean = keyboardHandler != null

    @JvmStatic
    fun areRepeatEventsEnabled(): Boolean = getKeyboard().areRepeatEventsEnabled()

    @JvmStatic
    fun enableRepeatEvents(enable: Boolean) = getKeyboard().enableRepeatEvents(enable)

    @JvmStatic
    fun getKeyName(key: Int): String = getKeyboard().getKeyName(key)

    @JvmStatic
    fun getEventKey(): Int = getKeyboard().getEventKey()

    @JvmStatic
    fun getEventCharacter(): Char = getKeyboard().getEventCharacter()

    @JvmStatic
    fun getEventKeyState(): Boolean = getKeyboard().getEventKeyState()

    @JvmStatic
    fun getEventNanoseconds(): Long = getKeyboard().getEventNanoseconds()

    @JvmStatic
    fun getNumKeyboardEvents(): Int = getKeyboard().getNumKeyboardEvents()

    @JvmStatic
    fun isRepeatEvent(): Boolean = getKeyboard().isRepeatEvent()

    @JvmStatic
    fun next(): Boolean = getKeyboard().next()

    @JvmStatic
    fun isKeyDown(key: Int): Boolean = getKeyboard().isKeyDown(key)
