...
import wtf.zani.spice.input.KeyboardImpl
import wtf.zani.spice.input.MouseImpl
import wtf.zani.spice.input.keyboardHandler
import wtf.zani.spice.input.mouseHandler
import wtf.zani.spice.util.Sync
...
object Display {
    ...
    fun setVSyncEnabled(vsync: Boolean) {
        if (Display.vsync != vsync) {
            logger.info("Turning VSync ${if (vsync) { "on" } else { "off" }}")
    ...
    fun setResizable(resizeable: Boolean) {
        if (handle != -1L && this.resizeable != resizeable) {
            glfwSetWindowAttrib(handle, GLFW_RESIZABLE, if (resizeable) { GLFW_TRUE } else { GLFW_FALSE })
        }
